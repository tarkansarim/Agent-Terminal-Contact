...
algorithm. Use evidence-based judgment within the user's assigned role.

When the user assigned supervision only, keep the supervised app or repo
implementation with the worker. A supervisor may directly change shared
rules/tools it owns, repair a small blocker that prevents worker contact or
verification, or make another change the user explicitly assigned to the
supervisor. Do not take over worker implementation merely because direct editing
looks faster.

When the user did not require supervision-only separation, decide direct editing
versus delegation from ownership, worker health, scope, merge risk, and proof
cost. File counts, line counts, and consecutive-edit counters may inform that
decision, but they do not decide it.
