...
class SkillContractTests(unittest.TestCase):
    ...
    def test_skill_requires_agent_contact_for_cross_agent_messages(self):
        ...
        self.assertIn("first prefer guarded\n  recovery for full payload residue", text)
        self.assertIn("require `would_submit_pending`", text)
        self.assertIn("submit the existing composer text without\n  pasting again", text)
        self.assertIn("require `clear_pending_guarded_contact`", text)
        self.assertIn("agent-tmux clear-input <session>", text)
        self.assertIn("rerun guarded `agent-contact`", text)
        ...
        self.assertIn("`--dry-run` must report `would_submit_pending`", text)
        self.assertIn("report `clear_pending_guarded_contact`", text)
