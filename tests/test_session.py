...
class SessionDiscoveryTests(unittest.TestCase):
    ...
    def test_explicit_sidecar_session_refuses_control_character_manifest_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            repo = tmp_path / "repo"
            artifact_dir = tmp_path / "sidecar-artifact"
            ...
            registry = artifact_dir.parent / ".agent-tmux-sidecar-registry" / f"{session}.txt"
            manifest = artifact_dir / "SIDECAR_REQUEST.txt"
            for request_file in (registry, manifest):
                request_file.write_text(
                    request_file.read_text(encoding="utf-8").replace(
                        "anchor=test-anchor\n",
                        "anchor=test\tanchor\n",
                    ),
                    encoding="utf-8",
